...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    if addr_tx_lists == 0 && valid_txs > 0 {
        ...
        println!(
            "   {} transactions involving test address are MISSING from database",
            2445 - our_count
        );
        println!(
            "   Action: Re-sync blockchain from PIVX daemon or check .dat files for corruption"
        );
