...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "✅ Found {} transactions in canonical chain ({} block entries)",
        canonical_txids.len(),
        block_count
    );
                ...
                if !key.is_empty() && key[0] == b't' && value.len() >= 8 {
                    ...
                    if height == 0 {
                        ...
                        if zero_height_txs.len() % 1000 == 0 {
                            println!(
                                "   Found {} height=0 transactions (scanned {})...",
                                zero_height_txs.len(),
                                tx_scanned
                            );
                        }
    ...
    println!(
        "✅ Found {} transactions with height=0 (scanned {} total)\n",
        zero_height_txs.len(),
        tx_scanned
    );
    ...
    for (tx_key, tx_value, txid) in &zero_height_txs {
        if canonical_txids.contains_key(txid) {
            ...
            if orphaned_count <= 5 {
                let txid_hex: Vec<u8> = txid.iter().rev().cloned().collect();
                println!(
                    "   ⚠️  Marked orphaned tx {} (not in canonical chain)",
                    hex::encode(&txid_hex)
                );
            }
        }
        ...
        if (orphaned_count + canonical_count) % BATCH_SIZE == 0 {
            ...
            if (orphaned_count + canonical_count) % 50_000 == 0 {
                println!(
                    "   Processed {} transactions ({} orphaned, {} fixed)...",
                    orphaned_count + canonical_count,
                    orphaned_count,
                    canonical_count
                );
            }
