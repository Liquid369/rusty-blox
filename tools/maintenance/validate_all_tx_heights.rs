...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "   ✅ Indexed {} canonical heights (0 → {})\n",
        canonical_heights.len(),
        max_height
    );
    ...
    for item in iter {
        ...
        if total_scanned % 500000 == 0 {
            println!(
                "   Scanned {total_scanned} transactions ({invalid_heights} invalid found)..."
            );
        }
    }
    ...
    println!(
        "🔧 Marking {} transactions as HEIGHT_ORPHAN (-1)...",
        to_mark_orphan.len()
    );
