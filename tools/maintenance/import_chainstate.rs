...
use rustyblox::config::{get_db_path, get_global_config, init_global_config};
use rustyblox::maturity::{filter_spendable_utxos, get_current_height};
use rustyblox::parser::{deserialize_transaction, deserialize_utxos};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !chainstate_path.exists() {
        eprintln!(
            "❌ Chainstate path does not exist: {}",
    ...
    if args.verify {
        ...
        let cf_names =
            DB::list_cf(&db_options, &db_path_str).unwrap_or_else(|_| vec!["default".to_string()]);
        ...
        println!(
            "⏳ Comparing {} addresses with explorer database...",
            balances.len()
        );
...
async fn get_address_balance(
    ...
    let spendable_utxos =
        filter_spendable_utxos(unspent_utxos.clone(), db.clone(), current_height).await;
...
fn print_verification_results(results: &VerificationResults) {
    ...
    if !results.examples.is_empty() {
        ...
        for (i, diff) in results.examples.iter().enumerate() {
            ...
            println!(
                "      Chainstate: {:.8} PIV",
                diff.chainstate as f64 / 100_000_000.0
            );
            println!(
                "      TxDB:       {:.8} PIV",
                diff.txdb as f64 / 100_000_000.0
            );
    ...
    println!(
        "\n📈 Match Rate: {:.2}% ({}/{})",
        match_rate, results.matches, total
    );
