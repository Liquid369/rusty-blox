...
### `count-orphans` - orphan diagnostics *(read-only)*
Count orphaned blocks/transactions and assess reorg impact. (`analyze-orphans`
lives in the `tools/diagnostics/` sub-crate, see below.)

---

## Sync validation

### `validate-sync` - full sync validation *(read-only)*
...
(`verify-txid-format`, `diagnose-tx`, `find-xpub-address`, `analyze-orphans`,
`trace-xpub`); run them from there:
`cd tools/diagnostics && cargo run --release --bin <name>`.
