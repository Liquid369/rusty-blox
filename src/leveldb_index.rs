...
pub fn build_canonical_chain_from_leveldb(
    ...
    let _span = info_span!("leveldb_import", db_path = leveldb_path).entered();
                    ...
                    if let Some(chainwork) = block_info.chainwork {
                        match &best_tip {
                            ...
                            Some((_, _, best_chainwork)) => {
                                if cmp_u256(&chainwork, best_chainwork)
    ...
    if blocks_without_chainwork > 0 {
        warn!(
            count = blocks_without_chainwork,
            "Blocks at tip heights missing chainwork"
        );
    }
    ...
    loop {
        let block_info = match index.get(&current_hash) {
            ...
            None => {
                warn!(
                    stopped_at_height = chain.last().map(|(h, _, _, _)| *h).unwrap_or(-1),
                    "Block not found - stopping chain walk"
                );
                break;
            }
        };
