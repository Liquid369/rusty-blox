use crate::chainstate::{
    aggregate_chainstate_with_coinbase_opts, parse_coins_value, AggregateOptions,
};
...
pub async fn enrich_from_chainstate(db: Arc<DB>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!(
        addresses = agg_result.balances.len(),
        coinbase_sats = agg_result.coinbase_total,
    ...
    info!(
        entries = chainstate_raw.len(),
        "UTXO entries found in chainstate"
    );

    let _cf_utxo = db.cf_handle("utxo").ok_or("utxo CF not found")?;
        ...
        if let Some(parsed) = parse_coins_value(value) {
            ...
            for (vout_index, amount, _script_pubkey, _kind, addresses) in parsed.unspent_outputs {
                ...
                for address in addresses {
                    ...
                    address_utxos.entry(address).or_default().push((
                        txid_internal.to_vec(),
                        actual_vout,
                        amount,
                    ));
                }
    ...
    info!(
        processed = processed,
        addresses = address_utxos.len(),
        skipped = skipped_no_script,
    ...
    for item in iter {
        ...
        if let Ok(tx) = deserialize_transaction(&tx_with_header).await {
            ...
            for input in &tx.inputs {
                ...
                if let Some(prevout) = &input.prevout {
                    if let Ok(prev_txid_bytes) = hex::decode(&prevout.hash) {
                        ...
                        if let Some(prev_tx_data) =
                            db.get_cf(&cf_transactions, &prev_tx_key).ok().flatten()
                        {
                            if prev_tx_data.len() >= 8 {
                                let prev_raw_tx = &prev_tx_data[8..];
                                let mut prev_with_header =
            ...
            for address in tx_addresses {
                address_txs.entry(address).or_default().insert(txid.clone());
            }
        }
        ...
        if tx_count % 100000 == 0 {
            debug!(
                tx_count = tx_count,
                "Transactions scanned for address lists"
            );
        }
    }
    ...
    info!(
        utxo_addresses = address_utxos.len(),
        tx_history_addresses = tx_list_written,
