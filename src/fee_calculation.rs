        ...
        TransactionType::Normal => {
            ...
            match calculate_input_value(db.clone(), &prevouts).await? {
                Some(value_in) => {
                    ...
                    if fee < 0 {
                        return Ok(FeeCalculation::failure(format!(
                            "Invalid fee: inputs ({value_in}) < outputs ({value_out})"
                        )));
                    }
...
pub async fn calculate_fee_by_txid(
    ...
    match tx_data {
        ...
        None => Ok(FeeCalculation::failure("Transaction not found".to_string())),
    }
}
...
mod tests {
    ...
    fn test_invalid_fee_negative() {
        ...
        let fee_calc =
            FeeCalculation::failure("Invalid fee: inputs (100) < outputs (200)".to_string());
