...
pub fn calculate_all_chainwork(
    ...
    info!(
        blocks = blocks_map.len(),
        "Calculating chainwork for all blocks"
    );
    ...
    info!(
        blocks = chainwork_map.len(),
        "Chainwork calculation complete"
    );
...
mod tests {
    ...
    fn test_work_calculation() {
        ...
        assert!(
            compare_chainwork(&higher_work, &work) == std::cmp::Ordering::Greater,
