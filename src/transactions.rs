use crate::address::{address_type_to_string, scriptpubkey_to_address};
use crate::batch_writer::BatchWriter;
use crate::constants::HEIGHT_UNRESOLVED;
use crate::db_utils::{perform_rocksdb_del, perform_rocksdb_get, perform_rocksdb_put};
use crate::parser::{deserialize_transaction, deserialize_utxos, reverse_bytes, serialize_utxos};
use crate::tx_type::{detect_type_from_components, TransactionType};
use crate::types::{
    AddressType, COutPoint, CScript, CTransaction, CTxIn, CTxOut, OutputDescription, SaplingTxData,
    SpendDescription,
};
...
async fn process_transaction_v1(
    ...
    let (_cf_transactions, _cf_pubkey, _cf_utxo) = if !fast_sync {
        ...
        let cf_pub = _db.cf_handle("pubkey").ok_or("pubkey CF not found")?;
        let cf_ut = _db.cf_handle("utxo").ok_or("utxo CF not found")?;
    ...
    let (tx_bytes, reversed_txid) = match tx_bytes_result {
        ...
        Err(e) => {
            ...
            let placeholder = format!("TRUNCATED_TX_{}_{}", block_height.unwrap_or(0), tx_index);
    ...
    if !fast_sync {
        ...
        for tx_out in &transaction.outputs {
            ...
            let existing_data_option =
                perform_rocksdb_get(_db.clone(), "pubkey", key_pubkey.clone()).await;
            if let Ok(Some(existing_data)) = existing_data_option {
                ...
                perform_rocksdb_put(_db.clone(), "pubkey", key_pubkey.clone(), serialized_utxos)
                    .await
                    .ok();
            }
            ...
            perform_rocksdb_put(
                ...
                "utxo",
                key_utxo.clone(),
                serialize_utxos(&utxos_to_serialize).await,
            )
            .await
            .ok();
                ...
                for addr in &tx_out.address {
                    ...
                    match perform_rocksdb_get(_db.clone(), "addr_index", key_t.clone()).await {
                        Ok(Some(mut existing)) => {
                            ...
                            perform_rocksdb_put(_db.clone(), "addr_index", key_t.clone(), existing)
                                .await
                                .ok();
                        }
                        _ => {
                            ...
                            perform_rocksdb_put(_db.clone(), "addr_index", key_t.clone(), newb)
                                .await
                                .ok();
                        }
                    }
                    ...
                    if let Ok(Some(existing_r)) =
                        perform_rocksdb_get(_db.clone(), "addr_index", key_r.clone()).await
                    {
                        if existing_r.len() == 8 {
                            current_received =
                                i64::from_le_bytes(existing_r[0..8].try_into().unwrap_or([0u8; 8]));
                        }
                    }
                    ...
                    perform_rocksdb_put(
                        ...
                        "addr_index",
                        key_r.clone(),
                        current_received.to_le_bytes().to_vec(),
                    )
                    .await
                    .ok();
                }
        ...
        for tx_in in &transaction.inputs {
            ...
            let tx_data_option =
                perform_rocksdb_get(_db.clone(), "transactions", key.clone()).await;
                    ...
                    if let Some(prevout) = &tx_in.prevout {
                        ...
                        if !output.address.is_empty() {
                            for addr in &output.address {
                                ...
                                let mut key_t = vec![b't'];
                                key_t.extend_from_slice(addr.as_bytes());
                                match perform_rocksdb_get(_db.clone(), "addr_index", key_t.clone())
                                    .await
                                {
                                    Ok(Some(mut existing)) => {
                                        existing.extend_from_slice(&reversed_txid);
                                        let _ = perform_rocksdb_put(
                                            _db.clone(),
                                            "addr_index",
                                            key_t.clone(),
                                            existing,
                                ...
                                if let Ok(Some(existing_s)) =
                                    perform_rocksdb_get(_db.clone(), "addr_index", key_s.clone())
                                        .await
                                {
                                    if existing_s.len() == 8 {
                                        current_sent = i64::from_le_bytes(
                                            existing_s[0..8].try_into().unwrap_or([0u8; 8]),
                                        );
                                    }
                                }
                                ...
                                let _ = perform_rocksdb_put(
                                    ...
                                    "addr_index",
                                    key_s.clone(),
                                    current_sent.to_le_bytes().to_vec(),
                                )
                                .await;
                            }
                        }
                        ...
                        let _ =
                            remove_utxo_addr(_db.clone(), &address_type, &prevout.hash, prevout.n)
                                .await;
                    }
                }
            }
            ...
            if let Ok(Some(data)) =
                perform_rocksdb_get(_db.clone(), "pubkey", key_utxo.clone()).await
            {
                ...
                if !utxos.is_empty() {
                    perform_rocksdb_put(
                        ...
                        "pubkey",
                        key_pubkey.clone(),
                        serialize_utxos(&utxos).await,
                    )
                    .await
                    .ok();
                } else {
                    perform_rocksdb_del(_db.clone(), "pubkey", key_pubkey.clone())
                        .await
                        .ok();
                }
            }
            ...
            perform_rocksdb_del(_db.clone(), "utxo", key_utxo.clone())
                .await
                .ok();
        }
    } else {
...
async fn parse_sapling_tx_data(
    ...
    let serialized_data =
        bincode::serialize(&sapling_tx_data).map_err(|e| io::Error::other(e.to_string()))?;
    ...
    for tx_in in &inputs {
        ...
        if let Some(ref internal_hash) = &referenced_utxo_internal {
            let mut key_utxo = vec![b'u'];
            key_utxo.extend_from_slice(internal_hash);
            perform_rocksdb_del(_db.clone(), "utxo", key_utxo)
                .await
                .ok();
        }
    }
...
async fn parse_vshield_spends(
    reader: &mut BufReader<File>,
) -> Result<Vec<SpendDescription>, io::Error> {
...
async fn parse_payload_data(reader: &mut BufReader<File>) -> Result<Option<Vec<u8>>, io::Error> {
    ...
    if byte_count > MAX_PAYLOAD_SIZE {
        ...
        Err(io::Error::other("Payload size exceeds the maximum."))
    } else {
...
async fn set_end_pos<R: AsyncReadExt + AsyncSeek + Unpin>(
    reader: &mut R,
    start_pos: u64,
) -> Result<u64, io::Error> {
...
async fn handle_address(
    ...
    for address_key in &address_keys {
        ...
        let existing_data =
            perform_rocksdb_get(_db.clone(), "addr_index", key_address.clone()).await;
...
async fn remove_utxo_addr(
    ...
    for address_key in &address_keys {
        ...
        if !existing_utxos.is_empty() {
            ...
            perform_rocksdb_del(_db.clone(), "addr_index", key_address)
                .await
                .ok();
        }
    }
