    ...
    match db.get_cf(&cf_state, b"sync_height")? {
        ...
        None => {
            ...
            match db.get_cf(&cf_metadata, &height_key)? {
                ...
                None => Ok(SyncStatus::NeedInitialSync),
            }
        }
    }
}
...
fn refresh_canonical_metadata(db: &Arc<DB>) -> Result<i32, Box<dyn std::error::Error>> {
    ...
    info!(
        tip_height = leveldb_height,
        "Canonical metadata refreshed from LevelDB"
    );
    Ok(leveldb_height)
}
...
async fn run_initial_sync_leveldb(
    ...
    info!(
        blocks = chain_len,
        tip_height = leveldb_height,
        "Canonical chain built from LevelDB"
    );
    ...
    info!(
        sync_height = leveldb_height,
        "Set sync_height from LevelDB canonical chain"
    );
    ...
    if !metadata_complete {
        return Err(
            "FATAL: Canonical metadata incomplete - cannot start parallel processing.\n\
    ...
    info!(
        heights = chain_len,
        "Canonical metadata validated - complete and contiguous"
    );
    ...
    let validate_with_offset_indexer = config
        .get_bool("sync.validate_offset_indexing")
        .unwrap_or(false);
    if validate_with_offset_indexer {
        ...
        if let Err(e) =
            index_canonical_blocks_by_offset(blk_dir.clone(), db.clone(), state.clone()).await
        {
...
async fn run_post_sync_enrichment(
    ...
    let mut height_resolution_complete =
        match db.get_cf(&cf_state, b"height_resolution_complete")? {
    ...
    if use_block_index && !height_resolution_complete {
        ...
        if let Some(Ok((_key, _value))) = db
            .iterator_cf(&cf_transactions, rocksdb::IteratorMode::Start)
            .next()
        {
    ...
    info!(
        tip_height,
        "==================== SYNC COMPLETE - database ready, explorer live ===================="
    );
...
async fn rebuild_transaction_block_index(db: &Arc<DB>) -> Result<(), Box<dyn std::error::Error>> {
            ...
            Ok((key, value)) => {
                ...
                if !key.is_empty() && key[0] == b't' {
                    ...
                    if tx_count % 100_000 == 0 {
                        debug!(
                            tx_count = tx_count,
                            "Block index rebuild: transactions processed"
                        );
                    }
                }
            }
    ...
    debug!(
        ...
        "Transactions read; writing block transaction index"
    );
...
async fn run_live_sync(
    ...
    let _span = info_span!("live_sync", current_height = current_height).entered();
    ...
    if blocks_behind > 1000 {
        info!(
            blocks_behind = blocks_behind,
            "Catching up via blk files (faster than RPC for large gaps)"
        );
        ...
        let entries = if blocks_behind < 1000 {
            ...
            info!(
                blocks_behind = blocks_behind,
                "Very close to tip - skipping blk files, catching up via RPC"
            );
            Vec::new()
            ...
            info!(
                blocks_behind = blocks_behind,
                files = recent_files.len(),
                "Processing most recent blk files"
            );
            recent_files
            ...
            info!(
                blocks_behind = blocks_behind,
                files = all_paths.len(),
                "Processing ALL blk files (faster than RPC)"
            );
            all_paths
        };
        ...
        info!(
            blocks_behind = blocks_behind,
            "Few blocks behind - skipping blk file scan, catching up via RPC"
        );
    }
...
pub async fn run_sync_service(
    ...
    match get_sync_status(&db).await? {
        ...
        SyncStatus::Synced { height } => {
            ...
            let network_height = match db.get_cf(&cf_state, b"network_height")? {
                Some(bytes) => {
                    i32::from_le_bytes(bytes.as_slice().try_into().unwrap_or([0, 0, 0, 0]))
                }
