...
pub async fn supply_analytics(
    ...
    match compute_supply_analytics(&db, &params.range).await {
        Ok(data) => {
            cache
                .set_json(&cache_key, &data, Duration::from_secs(300))
                .await;
            // Refresh the long-lived backup used for stale-while-revalidate.
            cache
                .set_json(&stale_key, &data, Duration::from_secs(86_400))
                .await;
            Ok(Json(data))
        }
        Err(e) => {
...
pub async fn transaction_analytics(
    ...
    let result = tokio::task::spawn_blocking(
        move || -> Result<Vec<TransactionDataPoint>, Box<dyn std::error::Error + Send + Sync>> {
...
pub async fn staking_analytics(
    ...
    let result = tokio::task::spawn_blocking(
        move || -> Result<Vec<StakingDataPoint>, Box<dyn std::error::Error + Send + Sync>> {
...
pub async fn network_health_analytics(
    ...
    let result = tokio::task::spawn_blocking(
        move || -> Result<Vec<NetworkHealthDataPoint>, Box<dyn std::error::Error + Send + Sync>> {
...
pub async fn rich_list(
    ...
    let result = tokio::task::spawn_blocking(
        move || -> Result<Vec<RichListEntry>, Box<dyn std::error::Error + Send + Sync>> {
...
pub async fn wealth_distribution(
    ...
    let result = tokio::task::spawn_blocking(
        move || -> Result<WealthDistribution, Box<dyn std::error::Error + Send + Sync>> {
...
pub async fn coldstaking_analytics(
    ...
    let db_clone = db.clone();
    let range = params.range.clone();
    let result = tokio::task::spawn_blocking(move || read_coldstaking_series(&db_clone, &range))
        .await
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
