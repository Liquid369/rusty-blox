...
mod tests {
    ...
    fn test_invalid_xpub_format() {
        ...
        for invalid in invalid_xpubs {
            ...
            assert!(
                result.is_err(),
                "Invalid xpub '{invalid}' should fail to parse"
            );
        }
    }
    ...
    fn test_address_derivation() {
        ...
        for (chain, index) in test_cases {
            let result =
                crate::api::addresses::derive_address(&xpub, &secp, chain, index, xpub.depth);
            assert!(
                result.is_ok(),
                "Derivation should succeed for chain {chain} index {index}"
            );
            ...
            assert!(
                path.contains("m/44'/119'"),
                "Path should contain PIVX BIP44 prefix"
            );
            assert!(
                path.contains(&format!("/{chain}/")),
                "Path should contain chain"
            );
            assert!(
                path.ends_with(&format!("/{index}")),
                "Path should end with index"
            );
        }
    }
    ...
    async fn test_gap_limit_early_termination() {
        ...
        assert_eq!(
            scanned, 23,
            "Should scan exactly 23 addresses with gap limit 20"
        );
    }
    ...
    fn test_gap_limit_validation() {
        ...
        // Invalid gap limits
        assert!(
            validate_gap_limit(Some(0)).is_err(),
            "Zero gap limit should be invalid"
        );
        assert!(
            validate_gap_limit(Some(201)).is_err(),
            "Gap limit > 200 should be invalid"
        );
    }
    ...
    fn test_pivx_address_encoding() {
        ...
        assert!(
            addr.starts_with('D'),
            "PIVX mainnet address should start with 'D', got: {addr}"
        );
    }
    ...
    fn test_error_messages() {
        ...
        assert!(
            err_msg.contains("invalid") || err_msg.contains("decode"),
...
mod validation_tests {
    ...
    fn test_xpub_version_compatibility() {
        ...
        assert_eq!(
            BITCOIN_XPUB_VERSION, PIVX_XPUB_VERSION,
...
mod privacy_tests {
    ...
    fn test_xpub_redaction() {
        ...
        assert_eq!(
            redacted, "xpub6CUG...DVmz",
            "Xpub should be redacted for logging"
        );
        assert!(
            !redacted.contains("TWtTMmzX"),
            "Middle section should be hidden"
        );
        assert!(
            redacted.len() < xpub.len(),
            "Redacted xpub should be shorter"
        );
    }
    ...
    fn test_xpub_redaction_preserves_prefix() {
        ...
        assert!(
            redacted.starts_with("xpub"),
            "Redacted xpub should preserve prefix"
        );
    }
}
...
mod pagination_tests {
    ...
    fn test_tokens_pagination_defaults() {
        ...
        assert_eq!(
            query.tokens_page_size, 1000,
            "Default tokens_page_size should be 1000"
        );
    }
    ...
    fn test_pagination_calculation() {
        ...
        assert_eq!(
            total_pages, 3,
            "125 tokens with page size 50 should have 3 pages"
        );
