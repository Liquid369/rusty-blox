...
fn read_varint(cursor: &mut Cursor<&[u8]>) -> Result<u64, ChainstateError> {
    ...
    match byte[0] {
        ...
        _ => Err(ChainstateError::ParseError(
            "Invalid varint prefix".to_string(),
        )),
    }
}
...
fn decompress_script(data: &[u8]) -> Result<Vec<u8>, ChainstateError> {
    ...
    match script_type {
        ...
        0x00 => {
            if data.len() != 21 {
                return Err(ChainstateError::InvalidScript(format!(
                    "Invalid P2PKH length: {} (expected 21)",
                    data.len()
                )));
            }
        ...
        0x01 => {
            if data.len() != 21 {
                return Err(ChainstateError::InvalidScript(format!(
                    "Invalid P2SH length: {} (expected 21)",
                    data.len()
                )));
            }
        ...
        0x02 | 0x03 => {
            if data.len() != 33 {
                return Err(ChainstateError::InvalidScript(format!(
                    "Invalid P2PK compressed length: {} (expected 33)",
                    data.len()
                )));
            }
        ...
        0x04 | 0x05 => {
            if data.len() != 33 {
                return Err(ChainstateError::InvalidScript(format!(
                    "Invalid P2PK uncompressed length: {} (expected 33)",
                    data.len()
                )));
            }
        ...
        0x06 => {
            if data.len() != 41 {
                return Err(ChainstateError::InvalidScript(format!(
                    "Invalid P2CS length: {} (expected 41)",
                    data.len()
                )));
            }
    ...
    for bit_position in 0..64 {
        if (avail & (1u64 << bit_position)) != 0 {
            ...
            if remaining.is_empty() {
                return Err(ChainstateError::ParseError(format!(
                    "Unexpected end of data at output {bit_position}"
                )));
            }
            ...
            if remaining.len() < script_bytes {
                return Err(ChainstateError::ParseError(format!(
                    "Not enough data for script at output {}: need {}, have {}",
                    bit_position,
                    script_bytes,
                    remaining.len()
                )));
...
pub fn aggregate_by_address(raw_map: HashMap<String, Vec<u8>>) -> HashMap<String, u64> {
    ...
    if parse_errors > 0 {
        warn!(
            parse_errors = parse_errors,
            "Parse errors encountered (first 10 logged above)"
        );
    }
