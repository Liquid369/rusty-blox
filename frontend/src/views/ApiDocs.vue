...
const GROUPS = [
  {
    ...
    endpoints: [
      ...
      { m: 'GET', p: '/rawblock/{height|hash}', d: '{"hex":"<raw serialized block>"}.', ttl: '60s' },
      ...
      { m: 'GET', p: '/utxo/{addr|xpub}', q: 'confirmed=true · gap', d: 'Unspent outputs incl. 0-conf mempool UTXOs; outputs spent by pending txs are hidden. confirmed=true = confirmed view only. xpub form scans the derive window (gap default 20) and tags rows with address + path.', ttl: '30s + live overlay' },
      { m: 'GET', p: '/balancehistory/{addr}', q: 'groupBy(secs) · from · to · fiatcurrency', d: 'Buckets {time, txs, received, sent, sentToSelf, rates}. Sum received-sent to reconstruct the balance curve; it lands exactly on the address balance. Addresses past 250k txs get 422 (honest refusal, cap env-tunable).', ttl: '300s' },
      { m: 'GET', p: '/estimatefee/{blocks}', d: '{"result":"<PIV/kB>"}, floored at the 0.0001 relay minimum.', ttl: '15s' },
      { m: 'GET', p: '/feestats/{height|hash}', d: 'Per-block fee stats {txCount, totalFeesSat, averageFeePerKb, decilesFeePerKb}. Coinstakes excluded (their minted reward reads as a negative fee).', ttl: '60s' },
      ...
      { m: 'GET', p: '/tickers', q: 'currency · timestamp', d: 'Fiat rates {ts, rates:{usd,eur,btc}}; nearest ticker at/after the timestamp. Unknown currency: {"<cur>":-1}.', ttl: '60s' },
      { m: 'GET', p: '/tickers-list', q: 'timestamp', d: 'Available fiat currencies at a timestamp.', ttl: '60s' },
      { m: 'GET', p: '/multi-tickers/', q: 'timestamp=ts1,ts2 · currency', d: 'One ticker per requested timestamp (max 100).', ttl: '60s' },
      { m: 'GET', p: '(/api/rawtx/{txid})', d: 'Raw tx hex as a bare JSON string (unversioned Blockbook path; /api/status serves the status envelope).', ttl: 'none' },
      { m: '404', p: 'not served', d: 'block-filters, contract/rpcCall (EVM-only), tickers ?block, address filter=inputs|outputs, tx ?spending + spentTxId enrichment. These return JSON 404 / unknown method.', ttl: '' },
    ],
  },
  {
    title: 'WEBSOCKET',
    ...
    endpoints: [
      { m: 'WS', p: '/websocket', d: 'Requests {id, method, params} -> {id, data}. Methods: getInfo, getBlockHash, getBlock, getAccountInfo, getAccountUtxo, getTransaction, getTransactionSpecific, getBalanceHistory, getCurrentFiatRates, getFiatRatesForTimestamps, getFiatRatesTickersList, estimateFee, sendTransaction, subscribeNewBlock, subscribeNewTransaction, subscribeAddresses, unsubscribe*, ping. Address subscribers receive confirmed-block txs too (superset of the newBlockTxs opt-in).', ttl: '' },
  ...
  },
  {
    ...
    endpoints: [
      ...
      { m: 'GET', p: '/analytics/hodl', d: 'Unspent value by time since last on-chain move; staking respends reset a coin\'s age.', ttl: '300s' },
      { m: 'GET', p: '/analytics/coldstaking', q: 'range', d: 'P2CS delegation series. net_cumulative is a LEVEL; single large days dominate short-window deltas.', ttl: '300s' },
